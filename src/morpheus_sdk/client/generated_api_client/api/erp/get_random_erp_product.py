from http import HTTPStatus
from typing import Any, Optional, Union

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.error_response import ErrorResponse
from ...models.get_random_erp_product_response_200 import GetRandomERPProductResponse200
from ...types import Response


def _get_kwargs(
    world_id: str,
) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": f"/{world_id}/erp/products/random",
    }

    return _kwargs


def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Optional[Union[ErrorResponse, GetRandomERPProductResponse200]]:
    if response.status_code == 200:
        response_200 = GetRandomERPProductResponse200.from_dict(response.json())

        return response_200

    if response.status_code == 404:
        response_404 = ErrorResponse.from_dict(response.json())

        return response_404

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[ErrorResponse, GetRandomERPProductResponse200]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    world_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Response[Union[ErrorResponse, GetRandomERPProductResponse200]]:
    """Get random ERP product


    Retrieve a random product from the world environment for testing and demonstration purposes.

    **Core Features**:
    - **Random Selection**: Algorithmically random product selection
    - **World Scoping**: Random selection within specific world environment
    - **Test Data Support**: Ideal for testing and demonstration scenarios
    - **Complete Product Data**: Returns full product profile with all specifications

    **Use Cases**:
    - **Testing & Development**: Generate random test data for development
    - **Demo Scenarios**: Create realistic demo scenarios with random products
    - **Load Testing**: Use random product data for performance testing
    - **Data Sampling**: Statistical sampling of product data for analysis


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[ErrorResponse, GetRandomERPProductResponse200]]
    """

    kwargs = _get_kwargs(
        world_id=world_id,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    world_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Optional[Union[ErrorResponse, GetRandomERPProductResponse200]]:
    """Get random ERP product


    Retrieve a random product from the world environment for testing and demonstration purposes.

    **Core Features**:
    - **Random Selection**: Algorithmically random product selection
    - **World Scoping**: Random selection within specific world environment
    - **Test Data Support**: Ideal for testing and demonstration scenarios
    - **Complete Product Data**: Returns full product profile with all specifications

    **Use Cases**:
    - **Testing & Development**: Generate random test data for development
    - **Demo Scenarios**: Create realistic demo scenarios with random products
    - **Load Testing**: Use random product data for performance testing
    - **Data Sampling**: Statistical sampling of product data for analysis


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[ErrorResponse, GetRandomERPProductResponse200]
    """

    return sync_detailed(
        world_id=world_id,
        client=client,
    ).parsed


async def asyncio_detailed(
    world_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Response[Union[ErrorResponse, GetRandomERPProductResponse200]]:
    """Get random ERP product


    Retrieve a random product from the world environment for testing and demonstration purposes.

    **Core Features**:
    - **Random Selection**: Algorithmically random product selection
    - **World Scoping**: Random selection within specific world environment
    - **Test Data Support**: Ideal for testing and demonstration scenarios
    - **Complete Product Data**: Returns full product profile with all specifications

    **Use Cases**:
    - **Testing & Development**: Generate random test data for development
    - **Demo Scenarios**: Create realistic demo scenarios with random products
    - **Load Testing**: Use random product data for performance testing
    - **Data Sampling**: Statistical sampling of product data for analysis


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[ErrorResponse, GetRandomERPProductResponse200]]
    """

    kwargs = _get_kwargs(
        world_id=world_id,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    world_id: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Optional[Union[ErrorResponse, GetRandomERPProductResponse200]]:
    """Get random ERP product


    Retrieve a random product from the world environment for testing and demonstration purposes.

    **Core Features**:
    - **Random Selection**: Algorithmically random product selection
    - **World Scoping**: Random selection within specific world environment
    - **Test Data Support**: Ideal for testing and demonstration scenarios
    - **Complete Product Data**: Returns full product profile with all specifications

    **Use Cases**:
    - **Testing & Development**: Generate random test data for development
    - **Demo Scenarios**: Create realistic demo scenarios with random products
    - **Load Testing**: Use random product data for performance testing
    - **Data Sampling**: Statistical sampling of product data for analysis


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[ErrorResponse, GetRandomERPProductResponse200]
    """

    return (
        await asyncio_detailed(
            world_id=world_id,
            client=client,
        )
    ).parsed
