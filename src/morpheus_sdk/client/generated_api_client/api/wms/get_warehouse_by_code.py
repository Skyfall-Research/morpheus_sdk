from http import HTTPStatus
from typing import Any, Optional, Union

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.error_response import ErrorResponse
from ...models.get_warehouse_by_code_response_200 import GetWarehouseByCodeResponse200
from ...types import Response


def _get_kwargs(
    world_id: str,
    warehouse_code: str,
) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": f"/{world_id}/wms/warehouses/code/{warehouse_code}",
    }

    return _kwargs


def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Optional[Union[ErrorResponse, GetWarehouseByCodeResponse200]]:
    if response.status_code == 200:
        response_200 = GetWarehouseByCodeResponse200.from_dict(response.json())

        return response_200

    if response.status_code == 404:
        response_404 = ErrorResponse.from_dict(response.json())

        return response_404

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[ErrorResponse, GetWarehouseByCodeResponse200]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    world_id: str,
    warehouse_code: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Response[Union[ErrorResponse, GetWarehouseByCodeResponse200]]:
    """Get warehouse by code


    Retrieve warehouse by unique warehouse code for direct facility access.

    **Core Features**:
    - **Direct Access**: Get warehouse by auto-generated warehouse code
    - **Code-based Lookup**: Fast retrieval using slugified warehouse name
    - **Complete Data**: Returns full warehouse configuration and address
    - **Optimized Query**: Uses indexed warehouseCode field for performance

    **Use Cases**:
    - **Code Integration**: Access warehouses via human-readable codes
    - **System Integration**: External system references using warehouse codes
    - **URL-friendly Access**: Use slugified codes in user interfaces
    - **Quick Lookup**: Direct warehouse access without ID knowledge


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.
        warehouse_code (str):  Example: atlanta-distribution-center.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[ErrorResponse, GetWarehouseByCodeResponse200]]
    """

    kwargs = _get_kwargs(
        world_id=world_id,
        warehouse_code=warehouse_code,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    world_id: str,
    warehouse_code: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Optional[Union[ErrorResponse, GetWarehouseByCodeResponse200]]:
    """Get warehouse by code


    Retrieve warehouse by unique warehouse code for direct facility access.

    **Core Features**:
    - **Direct Access**: Get warehouse by auto-generated warehouse code
    - **Code-based Lookup**: Fast retrieval using slugified warehouse name
    - **Complete Data**: Returns full warehouse configuration and address
    - **Optimized Query**: Uses indexed warehouseCode field for performance

    **Use Cases**:
    - **Code Integration**: Access warehouses via human-readable codes
    - **System Integration**: External system references using warehouse codes
    - **URL-friendly Access**: Use slugified codes in user interfaces
    - **Quick Lookup**: Direct warehouse access without ID knowledge


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.
        warehouse_code (str):  Example: atlanta-distribution-center.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[ErrorResponse, GetWarehouseByCodeResponse200]
    """

    return sync_detailed(
        world_id=world_id,
        warehouse_code=warehouse_code,
        client=client,
    ).parsed


async def asyncio_detailed(
    world_id: str,
    warehouse_code: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Response[Union[ErrorResponse, GetWarehouseByCodeResponse200]]:
    """Get warehouse by code


    Retrieve warehouse by unique warehouse code for direct facility access.

    **Core Features**:
    - **Direct Access**: Get warehouse by auto-generated warehouse code
    - **Code-based Lookup**: Fast retrieval using slugified warehouse name
    - **Complete Data**: Returns full warehouse configuration and address
    - **Optimized Query**: Uses indexed warehouseCode field for performance

    **Use Cases**:
    - **Code Integration**: Access warehouses via human-readable codes
    - **System Integration**: External system references using warehouse codes
    - **URL-friendly Access**: Use slugified codes in user interfaces
    - **Quick Lookup**: Direct warehouse access without ID knowledge


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.
        warehouse_code (str):  Example: atlanta-distribution-center.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[ErrorResponse, GetWarehouseByCodeResponse200]]
    """

    kwargs = _get_kwargs(
        world_id=world_id,
        warehouse_code=warehouse_code,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    world_id: str,
    warehouse_code: str,
    *,
    client: Union[AuthenticatedClient, Client],
) -> Optional[Union[ErrorResponse, GetWarehouseByCodeResponse200]]:
    """Get warehouse by code


    Retrieve warehouse by unique warehouse code for direct facility access.

    **Core Features**:
    - **Direct Access**: Get warehouse by auto-generated warehouse code
    - **Code-based Lookup**: Fast retrieval using slugified warehouse name
    - **Complete Data**: Returns full warehouse configuration and address
    - **Optimized Query**: Uses indexed warehouseCode field for performance

    **Use Cases**:
    - **Code Integration**: Access warehouses via human-readable codes
    - **System Integration**: External system references using warehouse codes
    - **URL-friendly Access**: Use slugified codes in user interfaces
    - **Quick Lookup**: Direct warehouse access without ID knowledge


    Args:
        world_id (str):  Example: 507f1f77bcf86cd799439011.
        warehouse_code (str):  Example: atlanta-distribution-center.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[ErrorResponse, GetWarehouseByCodeResponse200]
    """

    return (
        await asyncio_detailed(
            world_id=world_id,
            warehouse_code=warehouse_code,
            client=client,
        )
    ).parsed
